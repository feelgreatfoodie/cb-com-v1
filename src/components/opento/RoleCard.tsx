...
export function RoleCard({ title, description, brings }: RoleCardProps) {
  return (
    <motion.div
      variants={fadeInUp}
      whileHover={{ y: -4 }}
      className="glass rounded-xl p-6 transition-shadow duration-300 hover:shadow-[0_0_30px_color-mix(in_srgb,var(--accent)_15%,transparent)]"
    >
      <h3 className="mb-3 font-mono text-lg font-bold tracking-wide text-accent">
        {title}
      </h3>
